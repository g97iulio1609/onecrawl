...
export class PlaywrightScraperAdapter implements ScraperPort {
  ...
  async scrape(
    ...
    try {
      ...
      const { result } = await scrapePage(page, url, startTime, {
        timeout, waitFor, waitForSelector, jsCode,
        shouldExtractMedia, shouldExtractLinks, shouldExtractMetadata, onProgress,
      });

      if (useCache) this.cache.set(cacheKey, { data: result, timestamp: Date.now() });

      onProgress?.({ phase: "complete", message: `Scraped ${result.content.length} chars`, url });
  ...
  async scrapeMany(
    ...
    const { concurrency = 3, retries = 2, retryDelay = 1000, onProgress, signal, ...scrapeOptions } = options;
    const startTime = Date.now();
    ...
    onProgress?.({ phase: "complete", message: `Completed: ${results.size} success, ${failed.size} failed`, url: urls[0]! });
  ...
  getName(): string { return "playwright"; }
